...
function get_producto_by_id($producto_id){
    $sql= "SELECT productos.marca_id, productos.id, productos.unidades_de_medida_id, productos.categoria_id, productos.tipo_de_venta_de_producto_id, productos.producto, productos.precio_menudeo, productos.codigo_de_barras, productos.precio_mayoreo, productos.cantidad_mayoreo, productos.referencia_por_unidad, productos.descripcion, productos.`status` FROM productos WHERE productos.`status` = 1 AND productos.id = $producto_id";
    return get_item($sql);
}
